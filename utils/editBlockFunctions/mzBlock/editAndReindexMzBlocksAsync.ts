import { editBlockDbAsync } from "@/db/blocks/editBlockDbAsync";
import { BaseBlock, Block, MZ_BLOCK_TYPE_ID } from "@/interfaces/Block";
import { MzBlock } from "@/interfaces/MzBlock";
import { db } from "@/db/db";

export async function editAndReindexMzBlocksAsync(
    blocks: Block[], 
    oldBlockId: number,
    newBlock: BaseBlock & MzBlock
): Promise<Block[]> {
    const updatedBlocks: Block[] = [];
    let sampleIndex: number = 1;
    for (const b of blocks) {
        if (b.blockTypeId !== MZ_BLOCK_TYPE_ID) {
            updatedBlocks.push(b);
            continue;
        }
        const updatedBlock: BaseBlock & MzBlock = (b.id === oldBlockId) ? {...newBlock, id: oldBlockId} : {...b};
        updatedBlock.sampleIndex = (updatedBlock.recoveryInPercentage === 0) ? -1 : sampleIndex++;
        updatedBlocks.push(updatedBlock);
    }
    await db.withTransactionAsync(async () => {
        for (const b of updatedBlocks) {
            if (b.blockTypeId !== MZ_BLOCK_TYPE_ID) {
                continue;
            }
            await editBlockDbAsync(b);
        }
    });
    return updatedBlocks;
}