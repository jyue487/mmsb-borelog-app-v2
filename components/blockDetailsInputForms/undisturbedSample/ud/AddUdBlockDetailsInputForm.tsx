...
import { Block, UD_BLOCK_TYPE } from "@/interfaces/Block";
...
export function AddUdBlockDetailsInputForm({ style, boreholeId, blocks, setBlocks, setIsAddNewBlockButtonPressed, ...otherProps }: AddUdBlockDetailsInputFormProps) {
  ...
  return (
    ...
    <Button
      ...
      onPress={() => {
        ...
        const undisturbedSampleIndex: number = (recoveryLengthInMetres === 0) ? -1 : blocks.filter((block: Block) => block.blockType === UD_BLOCK_TYPE && block.recoveryInPercentage > 0).length + 1;

        const newBlock: Block = {
          id: blocks.length + 1,
          blockTypeId: UD_BLOCK_TYPE_ID,
          blockType: UD_BLOCK_TYPE,
