...
export type MzBlockDetailsInputFormProps = ViewProps & {
  boreholeId: number;
  blocks: Block[];
...
export function MzBlockDetailsInputForm({ style, boreholeId, blocks, setBlocks, setIsAddNewBlockButtonPressed, ...otherProps }: MzBlockDetailsInputFormProps) {
    ...
    <GestureHandlerRootView style={{ gap: 20 }}>
      ...
      <Button
        ...
        onPress={() => {
          ...
          const mazierSampleIndex: number = (recoveryLengthInMetres === 0) ? -1 : blocks.filter((block: Block) => block.blockType === 'Mz' && block.recoveryLengthInMetres > 0).length + 1;

          const newMzBlock: Block = {
            id: blocks.length + 1,
          ...
          setBlocks(blocks => [...blocks, newMzBlock]);
