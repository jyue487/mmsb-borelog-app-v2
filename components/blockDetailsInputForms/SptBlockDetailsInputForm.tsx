...
import { checkNonNegativeFloat, checkNonNegativeInteger, stringToDecimalPoint } from '@/utils/numbers';
		...
		<GestureHandlerRootView style={{ gap: 20 }}>
			...
			<Button
				...
				onPress={() => {
					...
					if (!checkNonNegativeInteger(seatingIncBlows1Str)) {
						alert(`Error: seatingIncBlows1Str`);
						return;
					}
					if (!checkNonNegativeInteger(seatingIncPen1Str)) {
						alert(`Error: seatingIncPen1Str`);
						return;
					}
					if (parseInt(seatingIncBlows1Str) < 25) {
						if (!checkNonNegativeInteger(seatingIncBlows2Str)) {
							alert(`Error: seatingIncBlows2Str`);
							return;
						}
						if (!checkNonNegativeInteger(seatingIncPen2Str)) {
							alert(`Error: seatingIncPen2Str`);
							return;
						}
					}
					if (!checkNonNegativeInteger(mainIncBlows1Str)) {
						alert(`Error: mainIncBlows1Str`);
						return;
					}
					if (!checkNonNegativeInteger(mainIncPen1Str)) {
						alert(`Error: mainIncPen1Str`);
						return;
					}
					if (parseInt(mainIncBlows1Str) < 50) {
						if (!checkNonNegativeInteger(mainIncBlows2Str)) {
							alert(`Error: mainIncBlows2Str`);
							return;
						}
						if (!checkNonNegativeInteger(mainIncPen2Str)) {
							alert(`Error: mainIncPen2Str`);
							return;
						}
					}
					if (parseInt(mainIncBlows1Str) + parseInt(mainIncBlows2Str) < 50) {
						if (!checkNonNegativeInteger(mainIncBlows3Str)) {
							alert(`Error: mainIncBlows3Str`);
							return;
						}
						if (!checkNonNegativeInteger(mainIncPen3Str)) {
							alert(`Error: mainIncPen3Str`);
							return;
						}
					}
					if (parseInt(mainIncBlows1Str) + parseInt(mainIncBlows2Str) + parseInt(mainIncBlows3Str) < 50) {
						if (!checkNonNegativeInteger(mainIncBlows4Str)) {
							alert(`Error: mainIncBlows4Str`);
							return;
						}
						if (!checkNonNegativeInteger(mainIncPen4Str)) {
							alert(`Error: mainIncPen4Str`);
							return;
						}
					}
					if (!checkNonNegativeInteger(recoveryLengthInMillimetresStr)) {
